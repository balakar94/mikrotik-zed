...
### Variable Navigation (`textDocument/definition` + `textDocument/references`, `navigation.rs`)

V1 scope — document-local, deterministic, honest about limits. **Declarations** are the identifier token immediately following a `:local`/`:global` command token on a logical line that OPENS with that command (the same primitive `symbols.rs` uses for outline naming, factored as `navigation::declared_variable`, so outline and navigation cannot disagree; inline values like `:local x=1` belong to `x` only). **Usages** are bare `$name` references found by a quote-aware byte scan per logical line: `$` inside `"…"`/`'…'` is literal, an unquoted `#` ends the scan (comments), and a doubled `$$` reads as a literal dollar; `$()` expressions and quoted names (`$"my var"`) are out of scope for v1. **Definition choice rule** (total, deterministic): among same-name declarations prefer the LAST one at or before the request position (closest preceding binding, or the declaration itself when invoked from it); if none precedes, take the FIRST declaration of that name regardless of kind — only document position breaks ties. **References** return the chosen declaration first when `context.includeDeclaration` is true (required key — absent/mistyped → `-32602`, matching sibling strictness), then every usage in document order, capped at 1000 (`MAX_REFERENCES`). Word-at-cursor reuses hover's `find_word_start/find_word_end`, and the extracted word must overlap a real indexed occurrence, so a property sharing a variable's spelling never resolves. Contracts: definition → `-32602` malformed / null untracked or non-variable cursor / single `Location`; references → `-32602` malformed / `[]` untracked or non-variable cursor / flat `Location[]`. Documented v1 limitations: no cross-file resolution, no block-scope precision (a `:local` inside `{ … }` is treated as document-visible), hyphenated names untracked.
...
| Framing | `lsp/src/framing.rs` | golden inputs: valid frames, garbage/malformed/duplicate headers terminal, oversized drained, EOF semantics |
