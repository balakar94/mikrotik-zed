...
- **Staged (unreleased)**: upstream re-sync `c77198 → c043cd8f` (`nd-ping` description for `/tool/ping`), `MAX_DIAGNOSTICS=2000` semantic diagnostic count cap, settings-provided `MIKROTIK_PASS` warn-and-ignore guard, dead-code cleanup (`build_base_url`, `cfg_with_no_loopback`, `LineContext.last_token`), caps-registry indexing. See [CHANGELOG.md](CHANGELOG.md) `[Unreleased]`.
