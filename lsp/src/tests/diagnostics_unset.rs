//! Diagnostics — unset and read-only.
use crate::diagnostics::severity;
use crate::diagnostics::*;
use crate::menus::MenuData;
use std::sync::Arc;
fn validator_data() -> Arc<MenuData> {
    Arc::new(MenuData::from_toml_str(
        r#"
[[menus]]
path = "/validator/typed"
type = "Directory"
[[menus.arguments]]
name = "flag"
type = "bool"
required = true
[[menus.arguments]]
name = "count"
type = "num"
[[menus.arguments]]
name = "uptime"
type = "time"
[[menus.arguments]]
name = "hw"
type = "macAddr"
[[menus.arguments]]
name = "peer"
type = "ipAddr"
[[menus.arguments]]
name = "bits"
type = "ubit (1Mbps, 2Mbps)"
[[menus.arguments]]
name = "freeform"
type = ""
[[menus]]
path = "/validator/props"
type = "Directory"
[[menus.arguments]]
name = "name"
type = "string"
required = true
[[menus.arguments]]
name = "temp"
type = "string"
unset = true
[[menus.arguments]]
name = "fixed"
type = "string"
unset = false
[[menus.read_only]]
name = "serial"
type = "string"
description = "factory serial"
[[menus]]
path = "/validator/chain"
type = "Directory"
[[menus.arguments]]
name = "chain"
type = "enum"
[[menus]]
path = "/validator/filter"
type = "Directory"
[[menus.arguments]]
name = "chain"
type = "enum"
[[menus.arguments]]
name = "action"
type = "enum (accept | drop)"
"#,
    ))
}

// ── Unset guard: Hint-only ─────────────────────────────────────────

fn assert_validator_severity_contract(diags: &[crate::diagnostics::Diagnostic]) {
    const WARNING_CODES: &[&str] = &[
        "unknown-menu",
        "unknown-property",
        "missing-required",
        "duplicate-property",
        "unknown-command",
        "invalid-enum-value",
    ];
    const ERROR_CODES: &[&str] = &["unclosed-brace", "unmatched-brace", "unclosed-quote"];
    for d in diags {
        match d.code.as_deref() {
            Some(c) if WARNING_CODES.contains(&c) => {
                assert_eq!(d.severity, Some(severity::WARNING), "code {c}")
            }
            Some(c) if ERROR_CODES.contains(&c) => {
                assert_eq!(d.severity, Some(severity::ERROR), "code {c}")
            }
            Some("truncated") => {
                assert_eq!(d.severity, Some(severity::INFORMATION), "truncated hint")
            }
            Some(other) => assert!(
                d.severity == Some(severity::HINT) || d.severity == Some(severity::INFORMATION),
                "Validator diagnostic '{other}' must be Hint or Information, got {:?}",
                d.severity,
            ),
            None => panic!("every rsc-ls diagnostic carries a code, got {d:?}"),
        }
        assert_eq!(d.source.as_deref(), Some("rsc-ls"), "source tag contract");
    }
}

fn typed_data() -> MenuData {
    MenuData::from_toml_str(
        r#"
[[menus]]
path = "/demo/typed"
type = "Directory"
[[menus.arguments]]
name = "flag"
type = "bool"
[[menus.arguments]]
name = "count"
type = "num"
[[menus.arguments]]
name = "period"
type = "time"
[[menus.arguments]]
name = "mac"
type = "macAddr"
[[menus.arguments]]
name = "addr"
type = "ipAddr"
[[menus.arguments]]
name = "prefix"
type = "ipPrefix"
[[menus.arguments]]
name = "addr6"
type = "ip6Addr"
[[menus.arguments]]
name = "prefix6"
type = "ip6Prefix"
[[menus.arguments]]
name = "rates"
type = "ubit (1Mbps, 2Mbps, 5.5Mbps, 11Mbps)"
[[menus.arguments]]
name = "truncated-ubit"
type = "ubit (mcs-0, mcs-1, mcs-2, mcs-3, mcs-4, mcs-5, mcs-6, mcs-7, mcs-8, mcs-9, mcs-10, mcs-11, mcs-12, mcs-13, m..."
[[menus.arguments]]
name = "untyped"
type = ""
[[menus.arguments]]
name = "maybe-unset"
type = "string"
unset = true
[[menus.arguments]]
name = "sticky"
type = "string"
[[menus.read_only]]
name = "actual"
type = "string"
"#,
    )
}

fn codes_for(data: &MenuData, doc: &str) -> Vec<Diagnostic> {
    compute_diagnostics(data, doc, "file:///validator-typed.rsc")
}

fn has_code(diags: &[Diagnostic], code: &str) -> bool {
    diags.iter().any(|d| d.code.as_deref() == Some(code))
}
// ── ubit ────────────────────────────────────────────────────

#[test]
fn validator_unset_flag_round_trips_and_guard_is_hint_only() {
    let data = validator_data();
    let menu = data
        .menu_by_path
        .get("/validator/props")
        .expect("synthetic menu");
    // The dataset flag itself round-trips (non-vacuous pin).
    assert!(
        menu.arguments
            .iter()
            .find(|a| a.name == "temp")
            .unwrap()
            .unset
    );
    assert!(
        !menu
            .arguments
            .iter()
            .find(|a| a.name == "fixed")
            .unwrap()
            .unset
    );
    // Setting either property via `add` is ordinary; the guard fires on
    // the `unset` verb only.
    let diags = crate::diagnostics::compute_diagnostics(
        &data,
        "/validator/props add name=x temp=y fixed=z",
        "file:///validator-unset.rsc",
    );
    assert_validator_severity_contract(&diags);
    // Presence pin (M4): unsetting a property whose `unset` flag is false
    // surfaces one Hint; deleting the guard must fail this test.
    let diags = crate::diagnostics::compute_diagnostics(
        &data,
        "/validator/props unset 0 fixed",
        "file:///validator-unset-guard.rsc",
    );
    assert_validator_severity_contract(&diags);
    let hits: Vec<_> = diags
        .iter()
        .filter(|d| d.code.as_deref() == Some("non-unsettable-property"))
        .collect();
    assert_eq!(
        hits.len(),
        1,
        "unsetting `fixed` (unset=false) must hint once, got {diags:?}",
    );
    assert_eq!(hits[0].severity, Some(severity::HINT));
    assert_eq!(hits[0].source.as_deref(), Some("rsc-ls"));
    assert!(
        hits[0].message.contains("fixed"),
        "hint must name the property, got {:?}",
        hits[0].message,
    );
    // Unsettable properties stay silent.
    let diags = crate::diagnostics::compute_diagnostics(
        &data,
        "/validator/props unset 0 temp",
        "file:///validator-unset-ok.rsc",
    );
    assert!(
        !diags
            .iter()
            .any(|d| d.code.as_deref() == Some("non-unsettable-property")),
        "unsettable property must stay silent, got {diags:?}",
    );
}

// ── Read-only-write: Information-only ──────────────────────────────

#[test]
fn validator_readonly_write_is_information_only() {
    let data = validator_data();
    let diags = crate::diagnostics::compute_diagnostics(
        &data,
        "/validator/props add name=x serial=ABC123",
        "file:///validator-readonly.rsc",
    );
    // No Warning fires for the read-only column today (it is allowlisted).
    assert!(
        !diags
            .iter()
            .any(|d| d.severity == Some(severity::WARNING) && d.message.contains("serial")),
        "read-only write must not warn, got {diags:?}",
    );
    // Presence pin (M4): writing a read-only column surfaces one
    // Information diagnostic; deleting the rule must fail this test.
    let hits: Vec<_> = diags
        .iter()
        .filter(|d| d.code.as_deref() == Some("read-only-write"))
        .collect();
    assert_eq!(
        hits.len(),
        1,
        "read-only write must surface one notice, got {diags:?}",
    );
    assert_eq!(hits[0].severity, Some(severity::INFORMATION));
    assert_eq!(hits[0].source.as_deref(), Some("rsc-ls"));
    assert!(
        hits[0].message.contains("serial"),
        "notice must name the property, got {:?}",
        hits[0].message,
    );
    assert_validator_severity_contract(&diags);
}

#[test]
fn test_ubit_members_and_exclusion_silent() {
    let data = typed_data();
    for good in ["1Mbps", "1Mbps,11Mbps", "1Mbps, 2Mbps", "!11Mbps"] {
        let doc = format!("/demo/typed set rates={good}");
        let diags = codes_for(&data, &doc);
        assert!(
            !has_code(&diags, "invalid-ubit-value"),
            "ubit '{good}' must stay silent, got {diags:?}"
        );
    }
}
