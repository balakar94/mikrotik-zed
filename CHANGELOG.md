...
- **Firewall chain values (`data/overrides.toml`, `data/commands.toml`, `scripts/extract_commands.py`)**: all eight `/ip{,v6}/firewall/{filter,mangle,nat,raw}` menus plus `/interface/bridge/{filter,nat}` declare their upstream-documented `chain` values, so `chain=` completion returns the real chains (mangle: prerouting/input/forward/output/postrouting; filter: input/forward/output; nat: srcnat/dstnat/input/output; raw: prerouting/output; bridge nat: srcnat/dstnat) instead of a generic fallback. The curated common-value hints are now offered only when an enum has no documented members, so populated chains never mix in invalid suggestions. The override mechanism supports enriching an existing plain `enum` without touching upstream type/description text.
- **Property-value hover (`lsp/src/hover.rs`)**: hovering the value side of `name=value` returns a card for boolean literals, enum members with the accepted values (bounded), other typed values with the shared gloss/example, and `$variable` references; unattributable values stay silent, structural delimiters (`]]`, `)`, `,`, `;`) are trimmed from the label, and quoted strings keep their inner brackets.
...
- **Binary resolution (`src/lib.rs`, `src/release.rs`)**: the auto-download now looks up the release tagged with the extension's own version (`v<version>`) first and only falls back to the latest stable release when that release is missing or lacks this platform's asset. A published extension snapshot therefore keeps its matching `rsc-ls` even after a newer stable is released. Shims shipped up to 0.6.1 keep the old latest-first order and cannot be changed retroactively, so publishing a newer stable while an older registry pin is live still moves those users' server binary — ship the registry pin update promptly.
...
- **Highlighting inside continuations (`languages/rsc/highlights.scm`)**: verb-list words inside value lists (`policy=\` + `ftp,reboot,…`) no longer render as keywords — the bare-identifier verb override is anchored to the identifier that starts a `\`-continued line; direct identifiers on property-only continuation lines (e.g. a dotted `client-endpoint=` hostname) are values instead of sub-menu green; and quoted URLs, including strings split across a `\` continuation, render as `@string.special` like unquoted URLs. Comments inside a quoted `source="…"` remain string content by design (the value is one string token).
- **Regexp values (`languages/rsc/highlights.scm`)**: `regexp=` / `regex=` values (6 + 1 arguments in the dataset) render with `@string.regex` — distinct wherever the theme styles it (pink in Catppuccin, `#bf956a` in One Dark) — with `@string` as the fallback; plain strings and `comment=` values are unaffected. Escape sequences inside the regex stay part of the string token (no sub-token highlighting).
